...
public class OUR3DTest extends JPanel{
	...
	int middleScreenX = 1334 / 2;
	int middleScreenY = 533;
	...
	public static void main(String...string){
		...
		OUR3DTest OUR3DTest = new OUR3DTest();
	...
	private void loadImage(){
		try {
			File f = new File("./bin/com/jdo/OUR3d/OURimage.png");
			System.out.println("f.exists():" + f.exists() + " - " + f.getAbsolutePath());
			//Image bi = ImageIO.read(new File("C:\\jason\\generic_workspace\\javatest\\com\\jdo\\OUR3d\\OURimage.png"));
			//Image bi = ImageIO.read(new File("C:\\jason\\generic_workspace\\javatest\\com\\jdo\\OUR3d\\Cottage Texture.jpg"));
			ImageHelper.add("OURDefaultImage", new File("./bin/com/jdo/OUR3d/OURimage.png"));
		}catch (Exception e){
			e.printStackTrace();
		}
	}
	...
	private void loadVertexs(){
		scale = 13.3f;//33;//53;//133 + 33*313;//;
		String workingLine = null;
		try {
			File[] files = new File("C:/jason/generic_workspace/models/to_draw").listFiles(new OBJFileFilter());
			for (File f : files){
		...
		public void run(){
			while (true){
				for (Polygon polygon : polygons){
					...
					//TexturerFillRedPattern.fillRasterizedImage(newIntImage, newFaces, newVertexs, polygon.textureCoordinates, ImageHelper.textureImageCache, middleScreenX, middleScreenY, depth, null);
